...
if (isset($_GET["action"])) {
 ...
 if ($_GET["action"]=="Grabar") {
  if ($id!="0") { // Es un usuario que ya existe en la base de datos
     $sqlcmd="UPDATE logout_usuarios SET usuario='".$Usuario."',id='".$Ids."',status='".$IdEm."',password= MD5('".$Contrasenia."') WHERE (id='".$_GET["id"]."')";
     mysqli_query($idCone,$sqlcmd);
     $mensaje="Registro actualizado.";
  } else { // Es un usuario nuevo...hay que agregarlo
      $Existe=0;
      $result = mysqli_query($idCone,"SELECT * FROM logout_usuarios WHERE (usuario LIKE '$Usuario')");
      while($row = mysqli_fetch_array($result)) { $Existe=1;  }
    if ($Existe!=1) mysqli_query($idCone,"INSERT INTO logout_usuarios VALUES ('".$Ids."','".$Usuario."',MD5('".$Contrasenia."'),'".$IdEm."')");
...
if (isset($_GET["id"])) { 
    $result = mysqli_query($idCone,"SELECT * FROM logout_usuarios WHERE id ='".$_GET["id"]."'");
   ...
   <td align="center">
	...
	<td>Numero</td>
     ...
     <?php
        $r=0;
  $idCone = conectarlocal();
	$cmdsql="SELECT * FROM logout_usuarios";
	if (isset($_GET["action"])) {
	  if ($_GET["action"]=="Buscar") {
	   $cmdsql="SELECT * FROM logout_usuarios WHERE usuario LIKE '%".$_GET["txtUsuario"]."%'";
	  }
	} 
        $result = mysqli_query($idCone,$cmdsql);
        while($row = mysqli_fetch_array($result)) {
	  if  ($r++ % 2) $bgcolor="#cde"; else $bgcolor="#fff;";
          echo "<tr style='background:$bgcolor'><td><a href='UserControl.php?id=".$row['id']."'>". $row['usuario'] . "</td><td>" . $row['password']."</td><td>". $row['status']."</td><td>".$row["id"]."</td></tr>";
        }
        mysqli_close($idCone);
     ?>
     </table>
